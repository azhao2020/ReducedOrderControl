...
% Model reduction method for unstable bogie system
...
%=========================================================%
simOut = sim('Model_Reduction_Scaling_Observer_UnStable_New','SimulationMode','normal',...
            'SaveState','on',...
            'SaveOutput','on',...
            'SaveFormat', 'Dataset');
u = simOut.get('ControlClosedLoopUnstable');
...
% swFigSize
% subplot(2,1,1)
% plot(tspan(100:80000),tau(100:80000,13),'--r','LineWidth',2);
% hold on
% plot(tspan(100:80000),tau(100:80000,14),'-.k','LineWidth',2);
% xlabel('Time (s)')
...
% 
% subplot(2,1,2)
% plot(tspan(100:80000), x_o_u(100:80000,1),'LineWidth',1.5)
% xlabel('Time (s)')
% ylabel('x(1)')
% legend('x(1)')
...
% % bar plot
% figure()
% bar_h1 = plot([1:length(idx)], hsvs1,'-*r',[length(idx)+1:length(hsvs)], hsvs2, 'o-b');
% set(gca,'XTick',[1:2:16])
% xlabel('State')
% ylabel('Hankel Singular Value $\sigma_i$')
% legend('Unstable modes','Stable modes', 'Location','NE')
